...
const url = "http://api.openweathermap.org/data/2.5/forecast/daily?id=5308655&appid=fbf712a5a83d7305c3cda4ca8fe7ef29&cnt=5";
...
app.get('/',(req,res) => {
    request(url1, (err,response,body) =>{
        if(err){
            ...
            console.log(message.copyright);
            //res.send(response);
            res.render('main',{message,title:'***Weather App***'});
        }
    });
});
